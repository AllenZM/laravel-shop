                ...
                <div class="card-body product-info">
                    ...
                    <div class="product-detail">
                        <ul class="nav nav-tabs">
                            <li class="nav-item">
                                <a class="nav-link active" href="#product-detail-tab" aria-controls="product-detail-tab" role="tab" data-toggle="tab">商品详情</a>
                            </li>
                            <li class="nav-item">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('.btn-favor').click(function () {
                // 发起一个 post ajax 请求，请求 url 通过后端 route() 函数生成。
            ...
            $('.btn-disfavor').click(function () {
                // 发起一个 delete ajax 请求，请求 url 通过后端 route() 函数生成。
            ...
            $('.btn-crowdfunding').click(function () {
                ...
                // 使用 jQuery 动态创建一个表单
                let $form = $('<form class="form-horizontal" role="form"></form>');
                // 表单中添加一个收货地址的下拉框
                $form.append('<div class="form-group row">' +
                    '<label for="email" class="col-sm-3 col-form-label text-md-right">选择地址</label>'+
                    '<div class="col-sm-9">' +
                ...
                // 在表单中添加一个名为购买数量的输入框
                $form.append('<div class="form-group row">' +
                    '<label for="email" class="col-sm-3 col-form-label text-md-right">购买数量</label>' +
            ...
            let hdl = setInterval(function () {
                ...
                let hourDiff = startTime.diff(now, 'hours');
                let minDiff = startTime.diff(now, 'minutes') % 60;
                let secDiff = startTime.diff(now, 'seconds') % 60;
                // 修改按钮的文字
            ...
            $('.btn-seckill').click(function () {
                ...
                let addresses = {!! json_encode(Auth::check() ? Auth::user()->addresses : []) !!};
                // 使用 jQuery 动态创建一个下拉框
                let addressSelector = $('<select class="form-control"></select>');
                // 循环每个收货地址
                addresses.forEach(function (address) {
                    // 把当前收货地址添加到收货地址下拉框选项中
                ...
                swal({
                    ...
                    // 从地址列表中找出当前用户选择的地址对象
                    let address = _.find(addresses, {'id': parseInt(addressSelector.val())});
