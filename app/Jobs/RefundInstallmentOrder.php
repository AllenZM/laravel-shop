...
use Illuminate\Contracts\Queue\ShouldQueue; // 代表一个异步任务
    ...
    public function handle()
    {
        ...
        foreach ($installment->items as $item) {
            ...
            try {
                ...
                // 假如某个还款计划退款报错了，则暂时跳过，继续处理下一个还款的退款
                continue;
            }
        }
        ...
        switch ($item->payment_method) {
            case 'wechatpay':
                app('wechat_pay')->refund([
                    ...
                    'notify_url' => ngrok_url('installments/wechat_pay/refund_notify'),
                ]);
                // 将还款佳话退款状态改成退款中
                $item->update([
